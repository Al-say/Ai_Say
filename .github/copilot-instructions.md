...
- **Data Flow**: User input in views → APIManager sends requests to backend (configured via AppConfig.baseURL) → Parse TextEvalResp → Update UI
...
- **Debug**: Backend runs on configured server; check `AppConfig.baseURL`
